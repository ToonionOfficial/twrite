...
9. **`completion`** (supplemental, after `hooks`): the generic inline-completion surface on the hook system alone - an `@mention` popup with no feature flags. The [Inline Completion](recipe-completion.md) recipe explains the pattern step by step.
